...
for indFrame = 1:2%needleVideo.NumberOfFrames-1
    % Read in the images from the video
    currentFrameData = read(videoFile,indFrame);
    nextFrameData = read(videoFile,indFrame+1);
    
    % Just look at the region of interest
    currentFrameDataROI(:,:,indFrame) = currentFrameData(...
        roi_xind(1):roi_xind(3),...
        roi_yind(1):roi_yind(2),:);
    nextFrameDataROI(:,:,indFrame) = nextFrameData(...
        roi_xind(1):roi_xind(3),...
        roi_yind(1):roi_yind(2),:);
...
%     implay(needleVideoROI_FILT./max(max(max(needleVideoROI_FILT))))
    ...
    for indFrame = 1:videoFile.NumberOfFrames-1
        % Read in the images from the video
